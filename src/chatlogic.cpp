...
ChatLogic::ChatLogic()
{
    //// STUDENT CODE
    ////
    std::cout << "ChatLogic() constructor Class" << "\n";
...
ChatLogic::~ChatLogic()
{
    //// STUDENT CODE
    ////
    std::cout << "~ChatLogic() unique_prt _nodes and _edges deallocated" << "\n";
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                       ...
                       auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode>& node) { return node->GetID() == id; });
                        ...
                        {
                             //_nodes.emplace_back(new GraphNode(id));  // updated to smart pointer
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            //GraphEdge *edge = new GraphEdge(id);  // updated to use smart pointer
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get());  //update edge to access pointer content
                            (*parentNode)->AddEdgeToChildNode(std::move(edge));  //updated to move semantics from edge content to parentnode
                                                    }
                        ////
                        //// EOF STUDENT CODE
                    }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root   update it to access pointer content
            }
            else
            {
    ...
    rootNode->MoveChatbotHere(std::move(chatBot));  // Updated to move semantics to rootNode from _chatBot
